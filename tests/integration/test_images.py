...
class TestImageAPIPublic:
    def test_list_user_images_only(self, service_user_api_client, workspace_factory, image_factory):
        ...
        other_images = image_factory.create_batch(2, workspace=other_workspace)
    ...
    def test_retrieve_own_image(self, service_user_api_client, workspace_factory, image_factory):
        user_workspace = workspace_factory(user_id=999)
    ...
    def test_cannot_access_others_image(self, service_user_api_client, workspace_factory, image_factory):
    ...
    def test_delete_own_image(self, service_user_api_client, workspace_factory, image_factory, temp_image_file):
        user_workspace = workspace_factory(user_id=999)
...
class TestImageAPIUnauthorized:
    ...
    def test_public_images_access_denied(self, api_client, image_factory, method, url, payload):
        image = image_factory()
    ...
    def test_internal_images_access_denied(self, api_client, image_factory, method, url, payload):
        image = image_factory()
        url = url.format(uuid=image.uuid)
        resp = getattr(api_client, method)(url, json=payload)
        assert resp.status_code in (401, 403)

...
class TestNodeODMClient:
    def test_for_task_creates_client_with_correct_attributes(
        self, mock_odm_server, settings
    ):
    ...
    def test_uuid_header_injection(
        self, task_uuid, mock_odm_server, endpoint, should_inject
    ):
    ...
    def test_create_task_injects_webhook_with_signature(
        self, nodeodm_client, task_uuid, settings
    ):
    ...
    def test_download_zip(self, mock_odm_server, initialized_task, tmp_path):
        mock_odm_server.manager.get_task(
            initialized_task.uuid
        ).status.code = 40  # COMPLETED

...
class GeoreferencingStagePublic(StageModel):
    ...
    pc_csv: bool = Field(
        False,
        alias="pc-csv",
        description="Export point cloud also in CSV format."
    )
    pc_ept: bool = Field(
        False,
        alias="pc-ept",
    ...
    pc_las: bool = Field(
        False,
        alias="pc-las",
        description="Export point cloud in LAS/LAZ format."
    )
...
class DEMStagePublic(StageModel):
    ...
    dtm: bool = Field(
        False,
        description="Generate Digital Terrain Model (bare-earth, ground only)."
    )
...
class DEMStageInternal(DEMStagePublic):
    ...
    cog: bool = Field(
        False,
        description="Create Cloud-Optimized GeoTIFFs (COG) for DEM/DSM/DTM."
    )
    tiles: bool = Field(
        False,
        description="Generate web-friendly tiled format (MBTiles) for DEM/DSM."
    )
